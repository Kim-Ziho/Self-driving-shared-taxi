...
class lc_path_pub :
    ...
    def getLaneChangePath(self,ego_path,lc_path,start_point,end_point,start_next_point, end_waypoint_idx): ## 
        ...
        #아마도.. 시작점에서 종료점까지의 거리..?
        start_path_distance = ((end_point.pose.position.x - start_point.pose.position.x)**2 + (end_point.pose.position.y - start_point.pose.position.y)**2)**0.5
        # 변수명이 이상하지만 시작점이 여러개라는 의미가 아니라 시작점부터 종료점까지 path의 수를 의미한다.
        ...
        for k in range(0,start_path_repeat+1):
            ...
            read_pose=PoseStamped()
            read_pose.pose.position.x = roation_matric_calc[0]
            read_pose.pose.position.y = roation_matric_calc[1]
