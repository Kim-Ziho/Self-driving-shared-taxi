...
import rospy
import rospkg
from math import cos,sin,pi,sqrt,pow,atan2, tan, tanh
...
class pure_pursuit :
    def __init__(self):
        ...
        # 1, 1로 하니 휘적휘적 운행되다가 결국 멈춘다. 적당히 2, 10으로 했을 때는 진행하기는 하였다.
    ...
    def calc_pure_pursuit(self):                
        ...
        # 회전 행렬의 역행렬 (반시계 회전행렬)
        # 반시계 회전행렬을 이용하는 이유는 자동차가 맵 좌표계에서 시계회전한 만큼 
        # Path좌표는 자동차에게 있어서 상대적으로 반시계만큼 회전한 것이기 때문

        #... 아니였다. 역행렬의 영어는 inverse, det는 뭔가 찾았더니 판별식 또는 행렬식(determinant)..
        # 아니 씹.. 그런데 아래의 방법도 틀린 것이 아니여서 고치질 못하겠다. 물론 global_path_point 변수명은 좀.. 이상하게 쓰고있다;
        ...
        for idx, pose in enumerate(self.path.poses) :
            ...
            # 자동차 좌표계를 기준으로 하므로 자동차의 위치만큼 - 를 해준다.
            # 참고로 행렬을 이용해서 반시계 회전행렬을 먼저하면 자동차 기준이 아닌 맵 기준 좌표계의 회전이 되버린다.
            # 그런데 왜 자동차 좌표계로 변환한 Path 변수명이 local_path_point..?
            # global_path_point이.. 변수명이 내가 의도한 대로 사용하지 않았나보다. 일단 변수명은 이상하지만 그대로 사용하겠다.
            global_path_point = [path_point.x - translation[0], path_point.y - translation[1], 1]
            local_path_point = det_trans_matrix.dot(global_path_point)    

            # arctan는 x가 0 이면 계산이 안되기 때문.. 인가..?
            if local_path_point[0] > 0 :
                # 전방주시거리(Look Forward Distance) 와 Path Point 거리
                dis = (local_path_point[0]**2 + local_path_point[1]**2)**0.5

                # 아.. 전방주시거리라는 것이 자동차의 전방을 말하는 것이 아니라 
                # 자동차를 중심으로 특정 거리 밖의 가장 가까운 Path를 말하는.. 건가..?
                if dis >= self.lfd :
        ...
        # 예전에 사용하기로는 예외처리를 어느정도 내부에서 해주는 arctan으로 기억하고있다.
        # 그런데 수식이 맞는지 모르겠다..
        # 참고로 forward_point[0]은 x, [1]는 y, [2]는 z이다.
