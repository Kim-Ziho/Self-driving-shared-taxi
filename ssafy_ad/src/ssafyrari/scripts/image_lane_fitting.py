...
class IMGParser:
    def __init__(self, pkg_name = 'ssafyrari'):
        ...
        # print(self.is_gps)
        # if not self.is_gps
        # self.path_pub = rospy.Publisher('/lattice_path', Path, queue_size=30)
        ...
        while not rospy.is_shutdown():
            ...
            if self.img_bgr is not None and self.is_status == True:
                ...
                # print(self.is_gps)
                if not self.is_gps:
                    curve_learner.vehicle_pos_x = 0
                    curve_learner.vehicle_pos_y = 0
                lane_path = curve_learner.write_path_msg(x_pred, y_pred_l, y_pred_r)
                # rospy.loginfo(lane_path)
